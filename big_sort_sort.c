...
/*   Updated: 2024/01/11 15:23:42 by yaharkat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

void sort_b_till_3(t_stack_node **a, t_stack_node **b, bool checker)
{
	int i;
	t_stack_node *tmp;
	...
	while (stack_size(*a) > 3 && !is_stack_sorted(*a))
	{
		...
		while (i >= 0)
		{
			if (i == calc_rarb(*a, *b, tmp->value))
				i = apply_rarb(a, b, tmp->value, 'a', checker);
			else if (i == calc_rrarrb(*a, *b, tmp->value))
				i = apply_rrarrb(a, b, tmp->value, 'a', checker);
			else if (i == calc_rarrb(*a, *b, tmp->value))
				i = apply_rarrb(a, b, tmp->value, 'a', checker);
			else if (i == calc_rrarb(*a, *b, tmp->value))
				i = apply_rrarb(a, b, tmp->value, 'a', checker);
			else
				tmp = tmp->next;
		}
	}
...
t_stack_node **sort_a(t_stack_node **a, t_stack_node **b, bool checker)
{
	int i;
	t_stack_node *tmp;
	...
	while (*b)
	{
		...
		while (i >= 0)
		{
			if (i == calc_rarb_a(*a, *b, tmp->value))
				i = apply_rarb(a, b, tmp->value, 'b', checker);
			else if (i == calc_rarrb_a(*a, *b, tmp->value))
				i = apply_rarrb(a, b, tmp->value, 'b', checker);
			else if (i == calc_rrarrb_a(*a, *b, tmp->value))
				i = apply_rrarrb(a, b, tmp->value, 'b', checker);
			else if (i == calc_rrarb_a(*a, *b, tmp->value))
				i = apply_rrarb(a, b, tmp->value, 'b', checker);
			else
				tmp = tmp->next;
		}
	}
